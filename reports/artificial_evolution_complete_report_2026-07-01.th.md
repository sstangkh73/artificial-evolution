...
> **ขอบเขตของคำเคลม (สำคัญ):** *ขนาด* ของ lure (เช่น 63%) และรูปเส้นโค้ง **ขึ้นกับกฎการเรียนรู้เฉพาะของเรา** (`_food_worth_eating`: ชิมของใหม่ครั้งเดียว → แช่แข็งค่า → เกณฑ์ pickiness×best) — learner แบบอื่น (softmax/ε-greedy/optimistic-init) อาจให้ตัวเลขต่างไป จึงเคลมเฉพาะ **เชิงคุณภาพ** ("blend สูงกว่า staple ได้ → ถูกล่อ") ส่วน *ปริมาณ* เป็นภาพประกอบด้วย learner ของเรา (ยังไม่ทดสอบ learner มาตรฐานอื่น — เป็น future work) นอกจากนี้ผลรันในโหมด**อิ่ม** → **ยังไม่ได้พิสูจน์ต้นทุน fitness จริง** ของการถูกล่อ (agent ไม่หิว จึงไม่มีผลต่อการรอด/การถูกคัดที่แสดงให้เห็น) "อันตรายกว่า" จึงหมายถึงค่าที่รับรู้ ไม่ใช่การตายที่วัดได้
...
7. **ขอบเขตคำเคลม lure (rule-dependence):** ผลที่ *rule-independent* คือ **discrimination = 0** (analytic — ตัวประมาณค่าที่ key ด้วยชนิดใด ๆ แยกสภาพไม่ได้โดยโครงสร้าง) ส่วน *ขนาด* ของ lure (63% ฯลฯ) ผูกกับ gate เฉพาะของเรา (ชิมครั้งเดียว+แช่แข็ง+EMA) — **ยังไม่ทดสอบ learner มาตรฐานอื่น** (softmax/ε-greedy) จึงเคลม "per-type learning แพ้" ได้เฉพาะเชิงคุณภาพ/เชิงตัวแทน ไม่ใช่ว่า *ทุกกฎ* ให้ตัวเลขเดียวกัน
