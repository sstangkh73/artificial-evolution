...
**โค้ดทั้งหมดอยู่ใน repo (commit ขึ้น `main`):** เครื่องมือ/knob ฝังใน `agents/agent.py`, `world/environment.py`, `world/metabolism.py`, `scripts/run_long_emergence_watch.py` (commit หลัก `cef1af4`, `fd47f3e`) · driver: `scripts/food_value_study_driver.py` · ตัวสร้างกราฟ: `scripts/make_food_value_figures.py` · knob ทั้งหมด default-neutral → v1 byte-identical
ความถูกต้องของผล: ผลเป็น deterministic (ดู §2.7) จึง reproducible เป๊ะด้วยคำสั่งด้านล่าง (รันจาก repo root)

```bash
# งบพลังงาน (รูปที่ 1)
python scripts/food_value_study_driver.py --model v2 --ticks 3000 --dump base.json
# การปลดล็อกการสืบพันธุ์ (รูปที่ 3) — body 38 มีเกราะ
python scripts/food_value_study_driver.py --model v2 --ticks 3000 --body 38 --drain-mult 0.05 --food-energy-mult 50 --dump births.json
# ผลหลัก: A (ไม่เรียนรู้) เทียบ B (เรียนรู้) ในสภาพอิ่ม (รูปที่ 4)
