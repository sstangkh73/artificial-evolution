...
SOURCE = ROOT / "reports" / "proposal" / "TURC2026_Concept_Proposal_Chisanuong_Injun_EN.md"
OUT = ROOT / "reports" / "proposal" / "TURC2026_Concept_Proposal_Chisanuong_Injun_EN.pdf"
...
def add_page_furniture(canvas, doc) -> None:
    ...
    canvas.drawCentredString(width / 2, 0.55 * cm, f"Page {doc.page}")
    canvas.restoreState()
...
def frontmatter(markdown: str) -> dict[str, str]:
    ...
    category = re.search(r"^Project category:\s*(.+)$", markdown, flags=re.MULTILINE)
    keywords = re.search(r"^Keywords:\s*(.+)$", markdown, flags=re.MULTILINE)
    member_block = markdown.split("## 2) Team Members", 1)[1].split("---", 1)[0]
    member_lines = [line.strip() for line in member_block.splitlines() if line.strip()]
    student_idx = member_lines.index("Team Leader")
    advisor_idx = member_lines.index("Advisor")
    return {
        "english_title": bold_lines[0],
        "category": category.group(1).strip() if category else "",
        "keywords": keywords.group(1).strip() if keywords else "",
        "student_name": member_lines[student_idx + 1].replace("Name: ", ""),
        "student_level": member_lines[student_idx + 2].replace("Grade: ", ""),
        "advisor_name": member_lines[advisor_idx + 1].replace("Name: ", ""),
        "advisor_school": member_lines[advisor_idx + 2].replace("School: ", ""),
        "advisor_affiliation": member_lines[advisor_idx + 3].replace("Affiliation: ", ""),
    }
...
def concept_diagram(styles: dict[str, ParagraphStyle]) -> Table:
    steps = [
        ...
        ("Basic world rules", "energy, plants, seeds, light, water, temperature, soil, food"),
        ("Agent-world interaction", "move, eat, remember locations, pick/drop seeds"),
        ("Experience-based knowledge metrics", "return lift, seed causal chain, patch productivity"),
        ("Control tests", "current-position, nearby, visible, random, low-food signal, world-size stress"),
        ("Evidence-bounded conclusion", "emergent ecological interaction / not yet intentional farming or full technology"),
    ]
    rows = []
    box_rows = []
...
def collect_variable_rows(lines: list[str], start: int) -> tuple[list[tuple[str, list[str]]], int]:
    labels = {"Independent variables:", "Dependent variables:", "Controlled variables:"}
    ...
    while idx < len(lines):
        ...
        if line.startswith("|"):
            ...
            while idx < len(lines) and lines[idx].strip().startswith("|"):
                ...
                idx += 1
            story.append(para("Summary of preliminary evidence from project experiments", styles["caption"]))
        ...
        if line.startswith("- "):
            ...
            continue
        if line in {"Independent variables:", "Dependent variables:", "Controlled variables:"}:
            flush_paragraph(paragraph_buffer, story, styles)
            rows, idx = collect_variable_rows(lines, idx)
            story.append(PageBreak())
            story.append(para("Research Variables", styles["h2"]))
...
def build() -> None:
    ...
    story.append(para("1) Project Title", styles["h1"]))
    story.append(
        para_lines(
            [
                "Studying Experience-Based Knowledge Emergence",
                "in Artificial Agents Placed in",
                "a Novel Simulated World",
            ],
            styles["title"],
        )
    )
    story.append(para(f'Project category: {info["category"]}', styles["meta"]))
    story.append(para(f'Keywords: {info["keywords"]}', styles["meta"]))
    story.append(spacer(6))

    story.append(para("2) Team Members", styles["h1"]))
